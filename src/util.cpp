...
const char * const BITCOIN_CONF_FILENAME = "bitevelin.conf";
const char * const BITCOIN_PID_FILENAME = "bitevelin.pid";
...
#else
    const char* pszModule = "bitevelin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Bitevelin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Bitevelin";
#else
    // Unix
    return pathRet / ".bitevelin";
#endif
#endif
}

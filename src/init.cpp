...
void Shutdown()
{
    ...
    RenameThread("bitevelin-shutoff");
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/bitevelin-project/bitevelin>";
    const std::string URL_WEBSITE = "<https://bitevelin.org>";
...
static void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("bitevelin-loadblk");
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if bitevelin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if bitevelin is started while in a temporary directory.\n",
